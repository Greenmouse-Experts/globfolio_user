...
export default function TopGroupsChart() {
  const { activeSub, isFree } = useRoutine();
  ...
  const currSub = subs?.data?.filter(
    (where: SubItemType) => where?.id === activeSub.planId
  );
  const globalSub = subs?.data?.filter(
    (where: SubItemType) => where?.name === "Global Plan"
  );
  ...
  const checkAccess = (item: ChatRoomItemType) => {
    ...
    if(!currSub[0].chatAccess.length){
      return (
        <p className="px-4 rounded-lg text-red-600 border border-red-600 fs-400 fw-600">
          Blocked
        </p>
      );
    }
    if (activeSub?.expiredAt && !isFree()) {
      if (!dayjs().isBefore(activeSub.expiredAt, "day")) {
        return (
          <p className="px-4 rounded-lg text-red-600 border border-red-600 fs-400 fw-600">
            Blocked
          </p>
    ...
    if (
      currSub[0]?.name === "Global Plan" ||
      currSub[0]?.name === "Hybrid Plan"
    ) {
      return (
        <p className="px-4 rounded-lg text-green-600 border border-green-6 fs-400 fw-60000">
          Accessible
        </p>
      );
    }
  ...
  const checkSub = (item: ChatRoomItemType) => {
    ...
    const filtered = subs?.data?.filter((array: SubItemType) =>
      accss.some((element: any) => array?.analystPickAccess?.includes(element))
    );
    return filtered;
  };
  return (
        ...
        <div className="overflow-x-auto">
          <div className="py-2 align-middle inline-block min-w-full ">
            <table className="items-center w-full bg-transparent border-collapse">
              ...
              <tbody>
                {groups &&
                  groups?.data.map((item: ChatRoomItemType) => (
                    <tr key={item.id}>
                      <td className="p-2 min-w-[200px] whitespace-nowrap">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="fs-200 lg:fs-400 fw-600 ">
                              {item.title}
                            </p>
                          </div>
                        </div>
                      ...
                      <td className="min-w-[120px] whitespace-nowrap">
                        <div className="flex">{checkAccess(item)}</div>
                      </td>
                      <td>
                        <div className="flex gap-2">
                          {!!checkSub(item)?.length &&
