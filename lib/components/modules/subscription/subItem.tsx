        ...
        <div className="px-4 py-12 grid gap-4">
            {
               item.benefits.map((item, i) => (
                <div className="flex justify-between">
                    <p>{item.benefit}</p>
                    <p><FaCircleCheck className="text-xl text-green-600 shrink-0"/></p>
                </div>
               )) 
            }
        </div>
        <div className="px-3 pb-5">
            <button className="bg-primary text-white fw-500 w-full py-3 rounded-[200px]" onClick={upgradeAction}>{isBusy? "Upgrading..." : "Get Started"}</button>
        </div>
      </div>
    <Modal title="" size="xs" >
